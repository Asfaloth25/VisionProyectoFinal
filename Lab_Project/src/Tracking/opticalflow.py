...
def stream_video():
    ...
    metric = None
    print('''
        Choose the metric you want to graph:
            - Acceleration: press A
            - Speed: press S
          ''')
    ...
    while True:
        ...
        if recording:
            ...
            if p1 is not None and st is not None:
                ...
                graph_data = [x[0] for x in (median_speeds, median_accelerations)[metric == 'accelleration'][-100:]]  # Keep only the last 50 points
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            ...
            break
        if cv2.waitKey(1) & (0xFF == ord('s') | 0xFF == ord('a')):
            recording = True
            metric = ('speed', 'accelleration')[0xFF == ord('a')]
